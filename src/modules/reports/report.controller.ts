...
router.get('/best-sellers', requirePermission('reports', 'read'), asyncHandler(async (req, res) => {
  const data = await service.bestSellers(req.params.tenantId, req.query);
  return res.status(200).json(data);
}));

router.get('/sales-by-period', requirePermission('reports', 'read'), asyncHandler(async (req, res) => {
  const data = await service.salesByPeriod(req.params.tenantId, req.query);
  return res.status(200).json(data);
}));

router.get('/profit-margin', requirePermission('reports', 'read'), asyncHandler(async (req, res) => {
