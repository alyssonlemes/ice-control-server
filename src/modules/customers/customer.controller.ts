...
router.post('/', requirePermission('customers', 'write'), asyncHandler(async (req, res) => {
  try {
...
router.get('/:customerId/orders', requirePermission('customers', 'read'), asyncHandler(async (req, res) => {
  const orders = await service.getCustomerOrders(req.params.tenantId, req.params.customerId);
  return res.status(200).json(orders);
}));

router.patch('/:customerId', requirePermission('customers', 'write'), asyncHandler(async (req, res) => {
  try {
...
router.delete('/:customerId', requirePermission('customers', 'admin'), asyncHandler(async (req, res) => {
  try {
