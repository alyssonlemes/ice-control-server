...
router.get('/cashier', requirePermission('financial', 'read'), async (req, res) => {
  const data = await service.getCashier(req.params.tenantId, req.query.date as any);
  return res.status(200).json(data);
});

router.post('/cashier/open', requirePermission('financial', 'write'), async (req, res) => {
  try {
...
router.post('/cashier/close', requirePermission('financial', 'write'), async (req, res) => {
...
router.get('/accounts-receivable', requirePermission('financial', 'read'), async (req, res) => {
  const data = await service.accountsReceivable(req.params.tenantId);
  return res.status(200).json(data);
});

router.get('/accounts-payable', requirePermission('financial', 'read'), async (req, res) => {
